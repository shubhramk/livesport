module.exports = function ( grunt ) {
  ...
  var taskConfig = {
    ...
    copy: {
      ...
      build_vendorjs: {
        files: [
          {
            src: [ '<%= vendor_files.js %>','<%= vendor_files.lazyLoadLibs %>' ],
            dest: '<%= build_dir %>/',
            cwd: '.',
            expand: true
          }
      ...
      compile_assets: {
        files: [
          ...
          {
            src: [ '<%= vendor_files.lazyLoadLibs %>' ],
            dest: '<%= compile_dir %>/',
            cwd: '.',
            expand: true
          },
          {
            src: [ 'src/modules/**/*.js','!src/**/*.spec.js' ],
            dest: '<%= compile_dir %>/',
            cwd: '.',
            expand: true
          }
    ...
    uglify: {
      compile: {
        ...
        files: [{
          expand: true,
          cwd: '<%= compile_dir %>',
          src: ['src/modules/**/*.js','!src/**/*.spec.js', 'assets/js/<%= pkg.name %>-<%= pkg.version %>.js'],
          dest:'<%= compile_dir %>'
        }]
      }
    },
  ...
  grunt.registerTask( 'build', [
    'clean', 'html2js', 'jshint', 'less:build', 'copy:build_appcache', 'copy:build_app_assets', 'copy:build_vendor_assets',
